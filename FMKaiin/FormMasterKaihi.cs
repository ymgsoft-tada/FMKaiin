...
//			AppCombo.SetComboBox(iKbnKaihi, enumKbn.DTypeShinryojo, true, (int)eTypeShinryojo.None);
			...
			DBView dvTmp = new DBView(AppGlobal.DB.GetFillTable(TableProp.t_bank_code));
...
//				{
//					iKbnKaihi.ExAddItem(dvTmp.CurrentRow.Row.ItemArray[2], 1);
//				}
//			}
...
//			gcom.Add(new GGridDBText(t_kaihi.FKaihi_BankCodeShiten, "支店コード", 60));
		...
		void changeFilter()
		{
			...
			{
				if (filter != "") filter += " AND ";
				filter += $"{t_kaihi.FID_KbnKkaihi} = {kbnKaihiId}";
			}

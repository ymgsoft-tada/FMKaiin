...
namespace App
{
	...
	public partial class t_bank_code : FieldProp
	{
		...
		/// フィールド[正式銀行名]。
		/// </summary>
		public const string F﻿BCD_FullName = "﻿BCD_FullName";
		/// <summary>
		/// 正式銀行名
		/// </summary>
		public string ﻿BCD_FullName
		{
			get	{	return Cast.String(row == null ? null : row[F﻿BCD_FullName]);	}
			set	{	_set(F﻿BCD_FullName, value);	}
		}
		...
		{
			get	{	if (row == null || row[F﻿BCD_FullName] == System.DBNull.Value) { return null; } else { return Cast.String(row[F﻿BCD_FullName]); }	}
			set	{	_set(F﻿BCD_FullName, value);	}
		}
		...
		public static DataTable GetTable()
		{
			...
			col = new DataColumn(F﻿BCD_FullName, typeof(string));
